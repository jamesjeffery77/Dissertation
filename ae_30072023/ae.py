...
folder_path = 'C:\\Users\\james\\autoencoder\\vae_use\\ae'
...
class AE(nn.Module):
    def __init__(self, n_classes=10):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, 10)  # directly produce the latent representation
        )
...
def train_model(model, train_loader, val_loader, EPOCHS,  optimizer):
    ...
    for epoch in range(EPOCHS):
        ...
        #evaluate the model on the validation data and get the metrics 
        metrics, *_ = evaluate_model(model, val_loader, optimizer, epoch)

        #latent space interpolation
        interpolate_in_latent_space(model, train_loader, 5, epoch, results_folder_path, 3, 9)        # NEW: Create a dictionary to store the metrics for this epoch
        ...
        # NEW: Write the metrics for this epoch to a CSV file
...
def evaluate_model(model, test_loader, optimizer, epoch):
    ...
    print("Transmitted Information AE: ", vae_trans_info)
...
def visualize_results(ae_losses,  val_ae_losses, original_imgs, reconstructed_imgs, encoded_images, targets, cluster_labels, vae_conf_mat):
    ...
    plt.savefig(os.path.join(results_folder_path, 'kmeans_cluster_plot.png'))  # Save plot instead of showing it
    plt.close(fig)
...
def train_perceptron(vae_model, perceptron, data_loader, optimizer, criterion):
    ...
    for data, targets in data_loader:
        data = data.view(data.size(0), -1)  # Reshape the data
        mu = vae_model.encode(data)  # We only care about mu
...
def interpolate_in_latent_space(model, dataset, num_steps, epoch, results_folder_path, class1, class2):
    ...
    latent_vector_1 = model.encode(data_point_1)
    latent_vector_2 = model.encode(data_point_2)
...
def main():
    ...
    # Write metrics to a csv file
    metrics_df = pd.DataFrame([metrics])  # Convert dict to DataFrame
    metrics_df.to_csv('aemetrics.csv', index=False)
